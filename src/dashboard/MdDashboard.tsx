...
import { Mdview_mdl3processesesService } from '../generated/services/Mdview_mdl3processesesService'
import type { Mdview_mdl3processeses } from '../generated/models/Mdview_mdl3processesesModel'
import { computeMetrics, peopleFromEvents } from './lib/compute'
import { peopleAnalytics, eventCountByMonth, type PeopleData } from './lib/peopleAnalytics'
import type { BoardData } from './lib/boardTypes'
import { BoardViewV2, type BoardSectionId } from './Board'
...
export default function MdDashboard({ section, onSectionChange }: { section?: BoardSectionId; onSectionChange?: (id: BoardSectionId) => void } = {}) {
  ...
  const [l3, setL3] = useState<Mdview_mdl3processeses[] | null>(null)
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      grab<Mdview_mdl3processeses>(Mdview_mdl3processesesService.getAll()),
    ]).then(([m, a, e, d, p]) => { if (!active) return; setMaster(m); setAgents(a); setEvents(e); setDivisions(d); setL3(p) })
    return () => { active = false }
  }, [])
  ...
  const board: BoardData | null = useMemo(() => {
    ...
    const b = computeMetrics(master, agents, l3 ?? [])
    if (events) {

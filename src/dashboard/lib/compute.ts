...
export function computeMetrics(masterRaw: Mdview_mdserviceses[], agentsRaw: Mdview_mdagentses[], l3Raw: Mdview_mdl3processeses[] = []): BoardData {
  ...
  // People is capped at 100 for the average (its overflow, e.g. 151 → "100% +51%",
  // is shown separately) so the gauge reads like the reference template (~43%).
  const gauge = avg([custPillar, procPillar, Math.min(PEOPLE_PILLAR, 100)].filter((x): x is number => x != null))
  ...
  return {
    ...
    // Agents by Pillar — FIXED counts for now. People shows 1225 active of 3714 created.
    portfolio: [
      { name: 'Services', value: 3, c: PILLAR_C.Customer },
      { name: 'Processes', value: 12, c: PILLAR_C.Processes },
      { name: 'People', value: 1225, total: 3714, c: PILLAR_C.People },
    ],

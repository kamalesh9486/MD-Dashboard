...
export function Tip({ text, tone = 'light' }: { text: string; tone?: 'light' | 'dark' }) {
  ...
  return (
    <span style={{ marginLeft: 'auto', flexShrink: 0, position: 'relative' }}
      ...
      {open && (
        <span style={{ position: 'absolute', top: 26, right: 0, zIndex: 30, width: 190, background: T.inkDeep, color: '#DCEFE5', font: `500 11px/1.5 ${BODY_FONT}`, padding: '9px 11px', borderRadius: 9, boxShadow: '0 12px 28px -8px rgba(0,0,0,.4)', textAlign: 'left' }}>{text}</span>
      )}
    </span>
  )
}
